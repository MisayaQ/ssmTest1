...
public class EmployeeController {
    ...
    public Msg getEmpsWithJson(@RequestParam(value = "pn", defaultValue = "1") Integer pn) {
        //引入pageHelper分页插件
        //在查询之前只需要调用 传入 要显示的页码 以及 每一页显示数据的条数
        PageHelper.startPage(pn, 8);
        //startPage后面紧跟的查询好就是一个分页查询
        List<Employee> emps = employeeService.getAll();
        //用PageInfo对结果进行包装 只需要将PageInfo交给页面就行了
        //封装了详细的分页信息 包括我们查询出来的数据,可以传入连续显示的页数
        PageInfo page = new PageInfo(emps, 8);
    ...
    public Msg checkeamil(@RequestParam("email") String email) {
    ...
    public Msg saveEmp(Employee employee, HttpServletRequest request) {
        System.out.println("请求体中的值：" + request.getParameter("gender"));//拿不到
